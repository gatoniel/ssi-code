...
from .folders import get_cache_folder
from ..log.log import lprint
from ..psf.simple_microscope_psf import SimpleMicroscopePSF
...
def add_blur_2d(image, k=17, sigma=5, multi_channel=False):
    ...
    if multi_channel:
        ...
        return numpy.moveaxis(
            numpy.stack([conv(channel) for channel in image]), 0, -1
        ), psf_kernel
    else:
